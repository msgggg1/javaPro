...
/**
 * @author msg
 * @date 2025. 3. 7. - 오후 3:19:41
 * @subject [ 제한된 제네릭 클래스 ]
 * @content		ㄴ T라는 타입 매개변수에 지정할 수 있는 타입의 종류를 제한.
...
//제네릭 클래스 선언
// T는 모든 타입(T)를 담을 수 있는 상자를 만드는 클래스 
class Box04<T>{
...
//[ 제한된 제네릭 클래스 선언 ]
class FruitBox<T extends Fruit>{ //과일을 물려받은 모든 타입들만 타입으로 사용하는 제네릭 클래스
	// 위의 멤버 복사 가정
}
...
//제네릭 클래스 선언.
// Eatable 인터페이스를 구현한 fruit 클래스의 자식 클래스만 타입으로 사용하겠다. 타입을 제한
