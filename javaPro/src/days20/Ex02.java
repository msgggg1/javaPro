	...
	public static void main(String[] args) {
		...
		*/
		...
		/* 내 풀이
		 ...
		 int [] nameIndex = new int [data.length()/"홍길동".length()];
		 
		 while ((data.indexOf("홍길동")) != -1) {
			 	
				index += sw==0? data.indexOf("홍길동"): 3+data.indexOf("홍길동");
				nameIndex[no] = index;
				sw = 1;
				System.out.printf("> %d번째 찾은 위치 : %d\n", ++no, index);
				data = data.substring(data.indexOf("홍길동") + "홍길동".length());
				
		} //  while
			 
//		  ㄴ. 두 번째 "홍길동"을 본인이름으로 바꿔서 출력하세요.
		 StringBuffer sb = new StringBuffer(originData);
		 sb.replace(nameIndex[1], nameIndex[1]+"홍길동".length(), "김민선");
		 System.out.println(sb.toString());
	*/
